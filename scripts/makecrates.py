...
description = "Support crates for {family} devices"
...
def main():
    ...
    for family in devices:
        ...
        cargo_toml = CARGO_TOML_TPL.format(
            family=family, crate=crate, version=VERSION, features=features)
        readme = README_TPL.format(
            family=family, crate=crate, device=devices[family][0],
            version=VERSION,
            devices="\n".join(d.upper().replace("X", "x")
                              for d in devices[family]))
        lib_rs = SRC_LIB_RS_TPL.format(family=family, mods=mods)
